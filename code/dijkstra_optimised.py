...
'''
Github Repositry Link : https://github.com/vinay06vinay/Path-Planning-of-A-Point-Robot-Using-Dijkstra-Algorithm
'''
...
def dijkstra(start_x,start_y,goal_x,goal_y):
    ...
    while(len(open_list)>0):
        current_cost,_,current_node=hq.heappop(open_list)
        if(goal_reached):
...
'''
Pygame is used in Visualising the Visited Nodes and thereafter the drawing the shortest path. Note That The origin is in top-left corner for pygame window
'''
def visualise_pygame(optimal_path,visited):
    ...
    for i in range(len(optimal_path) - 1):
        pygame.draw.polygon(s, (200, 0, 200), (optimal_path[i],optimal_path[i+1]),5)
        pygame.time.wait(10)
        pygame.display.update()
    while True:
